...
// modeStatus가 False일때 페이지.
class CasualRankingActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_casual_ranking)

        rankingRecyclerView = findViewById(R.id.casualRankingRecyclerView)
        myRankingTextView = findViewById(R.id.myCasualRanking)
        myNicknameTextView = findViewById(R.id.myCasualNickname)
        auth = FirebaseAuth.getInstance()

        database = FirebaseDatabase.getInstance().reference.child("casual_rankings")
    ...
    private fun loadRankings() {
        database.orderByChild("rank").addValueEventListener(object : ValueEventListener {
            ...
            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@CasualRankingActivity, "Failed to load rankings", Toast.LENGTH_SHORT).show()
            }
        })
    }
